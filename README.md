# 👋 Hi there! I'm Shaik Azeem

<!--
**azeem-shaik/azeem-shaik** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

🎓 I'm currently pursuing my **Master of Science (MSc) in Data Science**  
📊 Aspiring **Data Analyst** with a passion for transforming data into actionable insights

---

### 🧠 About Me
- 🔍 Strong analytical and problem-solving mindset
- 📈 Skilled in **Excel**, **Power BI**, **SQL**, and **Python**
- 🛠️ Hands-on experience with data wrangling, visualization, and storytelling
- 🧪 Love diving deep into data to uncover trends and patterns

---

### 📚 Projects
- 🧾 **Profit & Loss Dashboard** – Built in Excel with interactive slicers
- 📊 **Sales Analysis in Power BI** – Visual insights into product performance
- 🛒 **Consumer Goods Analytics with SQL** – Querying large datasets to drive decisions

---

---

### 📫 Connect With Me
- [LinkedIn](https://www.linkedin.com/in/shaik-azeem-z9901/)  
- Email: azeemshaikmohd29@gmail.com

---

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=your-github-username&show_icons=true&theme=radical)

